...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    /**
     * 인증이 성공했을 경우 호출되며, 사용자를 회원가입 시킨다.<br></br>
     * 이미 회원가입한 사용자에 한해 다른 경로로 리다이렉션 시키기 위해 <br></br>
     * {@link DefaultOAuth2UserService} 의 loadUser를 커스텀하였다. <br></br>
     * 데이터베이스에서 사용자를 찾고, attributes를 얻어서 nickname이 존재한다면 Map에 추가해준다. <br></br>
     * 이후에 호출되는 {@link com.windmeal.global.security.handler.CustomSuccessHandler} 에서 <br></br>
     * 닉네임의 존재 여부를 알 수 있고, 그에 따라 리다이렉션 로직을 분기할 수 있다.
     * @author Owen Choi
     ...
     */
    private void processDomain(Map<String, Object> userInfoAttributes, String token) throws OAuth2AuthenticationException {
...
//        String[] tokens = email.split("@");
    ...
    private Member registerIfNewUser(Map<String, Object> attributes) {
        Optional<Member> optionalMember = memberRepository.findByEmail((String)attributes.get("email"));
        Member member;
//        member = optionalMember.orElseGet(() -> memberRepository.save(createMember(oAuth2UserInfo)));
    ...
    /**
     * 아래의 로직들 (super..로 시작하는 메서드들)은 {@link OAuth2UserService} 인터페이스를 구현하는 {@link DefaultOAuth2UserService} 에서 가져옴.
     *<br></br>
     ...
     */
    private Map<String, Object> superClassLoadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    private ResponseEntity<Map<String, Object>> superClassGetResponse(OAuth2UserRequest userRequest, RequestEntity<?> request) {
        try {
