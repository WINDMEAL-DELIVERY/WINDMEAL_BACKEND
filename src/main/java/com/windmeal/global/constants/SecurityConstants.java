package com.windmeal.global.constants;

public class SecurityConstants {

    public static String ANONYMOUSUSER = "anonymousUser";
    // 모두 yml로 옮길 예정
    public static String ERROR_REDIRECT = "https://windmeal.co.kr/error";
    public static String DEFAULT_OAUTH_REDIRECT = "https://windmeal.co.kr";
    public static String NICKNAME_REDIRECT = "https://windmeal.co.kr/register";
    public static final String MISSING_USER_INFO_URI_ERROR_CODE = "missing_user_info_uri";
    public static final String MISSING_USER_NAME_ATTRIBUTE_ERROR_CODE = "missing_user_name_attribute";
    public static final String INVALID_USER_INFO_RESPONSE_ERROR_CODE = "invalid_user_info_response";
    public static final String NICKNAME_KEY = "nickname";

}
